...
export const updateUser = async (req, res, next) => {
  if (req.user.id !== req.params.id)
    return next(errorHandler(401, "Можете да променяте единствено Вашият профил!"));
  try {
...
export const deleteUser = async (req, res, next) => {
  if (req.user.id !== req.params.id)
    return next(errorHandler(401, "Можете да изтриете единствено Вашият профил!"));

  try {
    await User.findByIdAndDelete(req.params.id);
    res.clearCookie("access_token");
    res.status(200).json({ message: "Профилът Ви бе изтрит успешно!" });
  } catch (error) {
    next(error);
  }
};
...
export const getUserProperties = async (req, res, next) => {
  if (req.user.id === req.params.id) {
    ...
    return next(errorHandler(401, "Можете да преглеждате единствено собствените си обяви!"));
  }
};
...
export const sendContactForm = async (req, res, next) => {
  ...
  if (!username || !email || !message) {
    return next(errorHandler(400, "Всички полета са задължителни."));
  }
...
<body style="font-family: Arial, sans-serif; color: #333; margin: 0; padding: 0; background-color: #2d3748;"> <!-- Background color set to gray-800 -->
    ...
    mailTransporter.sendMail(mailDetails, function (err, data) {
      if (err) {
        return next(errorHandler(500, "Неуспешно изпращане на съобщението"));
      }
